...
void wait_in_ready(void) { while (! (UEINTX & (1 << TXINI))) ; }
void send_in(void) { UEINTX = ~(1 << TXINI); }
void wait_receive_out(void) { while (! (UEINTX & (1 << RXOUTI))) ; }

void ack_out(void) { UEINTX = ~(1 << RXOUTI); }
